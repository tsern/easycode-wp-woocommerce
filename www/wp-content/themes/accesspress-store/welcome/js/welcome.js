...
jQuery(document).ready(function ($) {
	...
	$(".install").on('click', function (e) {
    	...
    	var ajaxurl = accesspressstoreWelcomeObject.ajaxurl;

		$.ajax({
			type: 'POST',
			url: ajaxurl,
			data: {
				action: 'accesspressstore_plugin_installer',
				plugin: plugin,
				plugin_file: plugin_file,
				nonce: accesspressstoreWelcomeObject.admin_nonce,
			},
			success: function(response) {
		   		...
		   		if(response == 'success'){
				   		...
				   		el.html(accesspressstoreWelcomeObject.installed_btn);
	...
	$('.install-offline').on('click', function (e) {
		...
		$.ajax({
			...
			url: ajaxurl,
			data: {
				action: 'accesspressstore_plugin_offline_installer',
	...
	$(".activate").on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'accesspressstore_plugin_activation',
	   			plugin: plugin,
	   			nonce: accesspressstoreWelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
	   		success: function(response) {
		   		if(response){
	...
	$('.activate-offline').on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'accesspressstore_plugin_offline_activation',
	   			plugin: plugin,
	   			nonce: accesspressstoreWelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
